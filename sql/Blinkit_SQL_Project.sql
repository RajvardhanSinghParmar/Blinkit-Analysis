...
-- 1) How did monthly order value change compared with the previous month? 
...
-- 2) What is the cumulative (running) order value over each month?
...
-- 3) Which are the top 3 months based on total order value?
...
-- 4) Which months had the highest month-over-month increase and the highest month-over-month decline in order value?
...
-- 5) Who are the top 10 customers based on their total recorded order value, 
-- and what percentage of overall order value does each customer contribute?
...
-- 6) Which customer segments generate higher sales per customer?
...
-- 7) How many days does each customer wait between consecutive orders?
...
WITH Customer_Order_History AS (
    SELECT 
        ...
        bo.order_id,
        bo.order_date,
        LAG(bo.order_date) OVER (PARTITION BY bc.customer_id ORDER BY bo.order_date ASC) AS previous_order_date
    FROM blinkit_customers bc
    JOIN blinkit_orders bo 
        ON bc.customer_id = bo.customer_id
)
...
-- 8) For each customer, did their order value increase or decrease compared with their previous order?
...
WITH Customer_Order_History AS (
    SELECT 
        ...
        bo.order_date,
        bo.order_total,
        LAG(bo.order_total) OVER (PARTITION BY bc.customer_id ORDER BY bo.order_date ASC) AS previous_order_value
    FROM blinkit_customers bc
    JOIN blinkit_orders bo 
        ON bc.customer_id = bo.customer_id
)
...
-- 9) Which customers had a sequence of 3 consecutive orders with increasing order values?
...
WITH Order_History AS (
    SELECT
        ...
        bo.order_date,
        bo.order_total,
        LAG(bo.order_total, 1) OVER (PARTITION BY bc.customer_id ORDER BY bo.order_date) AS previous_order_value,
        LAG(bo.order_total, 2) OVER (PARTITION BY bc.customer_id ORDER BY bo.order_date) AS two_orders_back_value
    FROM blinkit_customers bc
    JOIN blinkit_orders bo
        ON bc.customer_id = bo.customer_id
),
...
-- 10) What are the top 3 products by sales value within each product category?
...
Product_Ranking AS (
    SELECT 
        ...
        category,
        total_sales_value,
        DENSE_RANK() OVER (PARTITION BY category ORDER BY total_sales_value DESC) AS product_rnk
    FROM Product_Sales
)
SELECT 
    product_id,
...
-- 11) What percentage of each category's total sales value is contributed by each product?
...
Category_Sales AS (
    SELECT 
        ...
        category,
        total_sales_value,
        SUM(total_sales_value) OVER (PARTITION BY category) AS category_overall_sales
    FROM Product_Sales
)
SELECT 
    product_id,
    ...
    ROUND((total_sales_value / category_overall_sales) * 100, 2) AS category_sales_percentage
FROM Category_Sales;


-- 12) Which products have sales value above the average product sales value of their category?
...
Category_Average_Sales AS (
    SELECT 
        ...
        category,
        total_sales_value,
        ROUND(AVG(total_sales_value) OVER (PARTITION BY category),2) AS category_average_sales
    FROM Product_Sales
)
SELECT *
...
-- 13) For each category, what is the difference between the top-selling product and the second-highest-selling product?
...
Product_Ranking AS (
    SELECT *, DENSE_RANK() OVER (PARTITION BY category ORDER BY total_sales_value DESC) AS product_rank
    FROM Product_Sales
),
Category_Top_Sales AS (
SELECT category,
MAX(CASE WHEN product_rank = 1 THEN product_name END) AS top_product_name,
MAX(CASE WHEN product_rank = 1 THEN total_sales_value END) AS top_product_sales,
MAX(CASE WHEN product_rank = 2 THEN product_name END) AS second_product_name,
MAX(CASE WHEN product_rank = 2 THEN total_sales_value END) AS second_product_sales
FROM Product_Ranking
GROUP BY category
)
SELECT 
...
-- 14) What percentage of total orders were delivered On Time, Slightly Delayed, and Significantly Delayed?
...
-- 15) Which deliveries perform better or worse than the overall average delivery time?

WITH Delivery_Average AS (
    SELECT 
        order_id,
        delivery_time_minutes,
        ROUND(AVG(delivery_time_minutes) OVER (), 2) AS overall_avg_delivery_time
    FROM blinkit_delivery_performance
),
Delivery_Comparison AS (
    SELECT 
        ...
        ROUND(delivery_time_minutes - overall_avg_delivery_time, 2) AS difference
...
-- 16) Which customers experienced repeated delayed deliveries?
...
-- 17) Which feedback categories have the highest negative sentiment rate?

WITH Category_Sentiment AS (
    SELECT 
        feedback_category,
        COUNT(order_id) AS total_feedback,
        COUNT(CASE WHEN sentiment = 'Negative' THEN order_id END) AS negative_feedback
...
SELECT feedback_category, 
	   total_feedback,
       negative_feedback,
    ROUND((negative_feedback / total_feedback) * 100, 2) AS negative_sentiment_rate
FROM Category_Sentiment
ORDER BY negative_sentiment_rate DESC;


-- 18) Does delivery performance affect customer ratings and sentiment?
...
WITH Delivery_Sentiment AS (
    SELECT 
        ...
        COUNT(CASE WHEN bc.sentiment = 'Positive' THEN 1 END) AS positive_feedback,
        COUNT(CASE WHEN bc.sentiment = 'Neutral' THEN 1 END) AS neutral_feedback,
        COUNT(CASE WHEN bc.sentiment = 'Negative' THEN 1 END) AS negative_feedback
...
-- 19) What percentage of total orders does each payment method account for?
...
SELECT
    ...
    total_orders, 
    overall_orders,
    ROUND((total_orders / overall_orders) * 100, 2) AS order_percentage
FROM Overall_Orders
ORDER BY order_percentage DESC;


-- 20) Which customer in each customer segment has generated the highest total sales?
...
Customer_Ranking AS (
    SELECT *,
        DENSE_RANK() OVER (PARTITION BY customer_segment ORDER BY total_sales DESC) AS segment_rank
    FROM Customer_Sales
)
SELECT *
FROM Customer_Ranking
